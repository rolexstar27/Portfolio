  ...
  return (
    <section id="contact" ref={ref} className="relative py-24 lg:py-32 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-grid-pattern bg-grid mask-radial opacity-50" />
        {/* Glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary-500/5 blur-[150px] rounded-full" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center mb-16 lg:mb-20">
            <span className="inline-flex items-center gap-2 text-primary-400 font-mono text-sm tracking-widest uppercase">
          ...
          <div className="grid lg:grid-cols-5 gap-12 lg:gap-16 max-w-5xl mx-auto">
            ...
            <motion.div variants={itemVariants} className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-4">
                {contactInfo.map(({ icon: Icon, label, value, href, color }) => (
                  <motion.div
                    key={label}
                    className="flex items-center gap-4 p-4 rounded-xl glass border-dark-800/50"
                    whileHover={{ x: 4 }}
                  >
                    <span className={`p-2.5 rounded-lg bg-dark-800/50 ${color}`}>
                      <Icon className="w-5 h-5" />
                    </span>
                    <div>
                      <p className="text-xs text-dark-500 uppercase tracking-wider">{label}</p>
                      {href ? (
                        <a href={href} className="text-sm text-dark-200 hover:text-primary-400 transition-colors">
                          {value}
                        </a>
                      ) : (
              ...
              {/* Social Links */}
              <div>
                <p className="text-xs text-dark-500 uppercase tracking-wider mb-3">Follow Me</p>
                <div className="flex items-center gap-3">
                  {socialLinks.map(({ icon: Icon, href, label, color }) => (
                    <motion.a
                      ...
                      href={href}
                      aria-label={label}
                      className={`w-10 h-10 rounded-full glass border-dark-700/50 flex items-center justify-center text-dark-400 ${color} transition-all duration-300`}
            ...
            <motion.div variants={itemVariants} className="lg:col-span-3">
              <div className="p-6 sm:p-8 rounded-3xl glass border-dark-800/50 relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="relative z-10 space-y-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        required
                        className="w-full px-4 py-3 rounded-xl bg-dark-900 border border-dark-700 text-white text-sm placeholder:text-dark-600 focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20 transition-all duration-300"
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      required
                      rows={5}
                      className="w-full px-4 py-3 rounded-xl bg-dark-900 border border-dark-700 text-white text-sm placeholder:text-dark-600 focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20 transition-all duration-300 resize-none"
